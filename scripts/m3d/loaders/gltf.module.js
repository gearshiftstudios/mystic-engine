...
/* import essentials */ 
import { EPATH } from '../../path.module.js'
...
const GLTFLoader = ( function () {
	...
	*/
	 ...
	 * @return {Promise<M3DREP.attribute.buffer|M3DREP.attribute.interleavedBuffer>}
	 */
	GLTFParser.prototype.loadAccessor = function ( accessorIndex ) {
		...
		return Promise.all( pendingBufferViews ).then( function ( bufferViews ) {
			...
			byteStride = accessorDef.bufferView !== undefined ? json.bufferViews[ accessorDef.bufferView ].byteStride : undefined,
			...
			if ( byteStride && byteStride !== itemBytes ) {
				...
				if ( ! ib ) {
					...
					ib = new M3DREP.interleavedBuffer( array, byteStride / elementBytes )

					parser.cache.add( ibCacheKey, ib )
				}

				bufferAttribute = new M3DREP.attribute.interleavedBuffer( ib, itemSize, ( byteOffset % byteStride ) / elementBytes, normalized )
			} else {
				...
				bufferAttribute = new M3DREP.attribute.buffer( array, itemSize, normalized )
			}
			...
			if ( accessorDef.sparse !== undefined ) {
				...
				if ( bufferView !== null ) bufferAttribute = new M3DREP.attribute.buffer( bufferAttribute.array.slice(), bufferAttribute.itemSize, bufferAttribute.normalized )
	 ...
	 * @return {Promise<M3DREP.texture.default>}
	 */
	GLTFParser.prototype.loadTexture = function ( textureIndex ) {
		...
		textureExtensions = textureDef.extensions || {},
		...
		if ( source.uri ) loader = options.manager.getHandler( source.uri )
	...
	GLTFParser.prototype.loadTextureImage = function ( textureIndex, source, loader ) {
		...
		return Promise.resolve( sourceURI ).then( function ( sourceURI ) {
			...
			sampler = samplers[ textureDef.sampler ] || {}
	 ...
	 * @param  {M3DREP.object3D} mesh Mesh, Line, or Points instance.
	 */
	GLTFParser.prototype.assignFinalMaterial = function ( mesh ) {
		var geometry = mesh.geometry,
		material = mesh.material,

		useVertexTangents = geometry.attributes.tangent !== undefined,
		useVertexColors = geometry.attributes.color !== undefined,
		useFlatShading = geometry.attributes.normal === undefined,
		useSkinning = mesh.isSkinnedMesh === true,
		useMorphTargets = Object.keys( geometry.morphAttributes ).length > 0,
		useMorphNormals = useMorphTargets && geometry.morphAttributes.normal !== undefined
		...
		if ( mesh.isPoints ) {
			...
			if ( ! pointsMaterial ) {
				pointsMaterial = new M3DREP.material.points()

				M3DREP.material.default.prototype.copy.call( pointsMaterial, material )
			...
			if ( ! lineMaterial ) {
				lineMaterial = new M3DREP.material.line.basic()

				M3DREP.material.default.prototype.copy.call( lineMaterial, material )
	...
	GLTFParser.prototype.getMaterialType = function ( /* materialIndex */ ) {
		return M3DREP.material.mesh.standard
	}

	/**
	 * Specification: https://github.com/KhronosGroup/glTF/blob/master/specification/2.0/README.md#materials
	 * @param {number} materialIndex
	 * @return {Promise<M3DREP.material.default>}
	 */
	GLTFParser.prototype.loadMaterial = function ( materialIndex ) {
		...
		if ( materialDef.normalTexture !== undefined && materialType !== M3DREP.material.mesh.basic ) {
		...
		if ( materialDef.occlusionTexture !== undefined && materialType !== M3DREP.material.mesh.basic ) {
		...
		if ( materialDef.emissiveFactor !== undefined && materialType !== M3DREP.material.mesh.basic ) materialParams.emissive = new Color().fromArray( materialDef.emissiveFactor )
		if ( materialDef.emissiveTexture !== undefined && materialType !== M3DREP.material.mesh.basic ) pending.push( parser.assignTexture( materialParams, 'emissiveMap', materialDef.emissiveTexture ) )
		...
		return Promise.all( pending ).then( function () {
			...
			if ( material.map ) material.map.encoding = M3DREP.encoding.srgb
			if ( material.emissiveMap ) material.emissiveMap.encoding = M3DREP.encoding.srgb
	...
	GLTFParser.prototype.createUniqueName = function ( originalName ) {
		...
		for ( var i = 1; this.nodeNamesUsed[ name ]; ++ i ) name = sanitizedName + '_' + i
	...
	/**
	 * @param {M3DREP.geometry.buffer.default} geometry
	...
	function computeBounds ( geometry, primitiveDef, parser ) {
		...
		if ( attributes.POSITION !== undefined ) {
			...
			if ( min !== undefined && max !== undefined ) {
				box.set(
					new M3DREP.vec3( min[ 0 ], min[ 1 ], min[ 2 ] ),
					new M3DREP.vec3( max[ 0 ], max[ 1 ], max[ 2 ] ) )
			} else {
		...
		if ( targets !== undefined ) {
			var maxDisplacement = new M3DREP.vec3(),
			vector = new M3DREP.vec3()
	 ...
	 * @param {GLTF.Primitive} primitiveDef
	 * @param {GLTFParser} parser
	 * @return {Promise<M3DREP.geometry.buffer.default>}
	 */
	 ...
	 * @return {M3DREP.geometry.buffer.default}
	 */
	 ...
	 * @return {Promise<Array<M3DREP.geometry.buffer.default>>}
	 */
	GLTFParser.prototype.loadGeometries = function ( primitives ) {
		...
		for ( let i = 0, il = primitives.length; i < il; i ++ ) {
			...
			if ( cached ) pending.push( cached.promise )
				...
				else geometryPromise = addPrimitiveAttributes( new M3DREP.geometry.buffer.default(), primitive, parser )
	 ...
	 * @return {Promise<M3DREP.group|M3DREP.mesh.default|M3DREP.mesh.skinned>}
	 */
			...
			for ( let i = 0, il = geometries.length; i < il; i ++ ) {
				...
				if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
					...
					mesh = meshDef.isSkinnedMesh === true
						? new M3DREP.mesh.skinned( geometry, material )
						: new M3DREP.mesh.default( geometry, material )

					if ( mesh.isSkinnedMesh === true && ! mesh.geometry.attributes.skinWeight.normalized ) mesh.normalizeSkinWeights()

					if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLE_STRIP ) mesh.geometry = toTrianglesDrawMode( mesh.geometry, M3DREP.triangleStripDrawMode )
				    else if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLE_FAN ) mesh.geometry = toTrianglesDrawMode( mesh.geometry, M3DREP.triangleFanDrawMode )
                } else if ( primitive.mode === WEBGL_CONSTANTS.LINES ) mesh = new M3DREP.line.segments( geometry, material )
				else if ( primitive.mode === WEBGL_CONSTANTS.LINE_STRIP ) mesh = new M3DREP.line.default( geometry, material )
				else if ( primitive.mode === WEBGL_CONSTANTS.LINE_LOOP ) mesh = new M3DREP.line.loop( geometry, material )
				else if ( primitive.mode === WEBGL_CONSTANTS.POINTS ) mesh = new M3DREP.points( geometry, material )
	 ...
	 * @return {Promise<M3DREP.camera.default>}
	 */
	GLTFParser.prototype.loadCamera = function ( cameraIndex ) {
		var camera,
		cameraDef = this.json.cameras[ cameraIndex ],
		...
		if ( cameraDef.type === 'perspective' ) camera = new M3DREP.camera.depth( M3DREP.util.math.radToDeg( params.yfov ), params.aspectRatio || 1, params.znear || 1, params.zfar || 2e6 )
		else if ( cameraDef.type === 'orthographic' ) camera = new M3DREP.camera.flat( - params.xmag, params.xmag, params.ymag, - params.ymag, params.znear, params.zfar )
	 ...
	 * @return {Promise<M3DREP.animation.clip>}
	 */
	GLTFParser.prototype.loadAnimation = function ( animationIndex ) {
		...
		for ( let i = 0, il = animationDef.channels.length; i < il; i ++ ) {
			...
			input = animationDef.parameters !== undefined ? animationDef.parameters[ sampler.input ] : sampler.input,
			output = animationDef.parameters !== undefined ? animationDef.parameters[ sampler.output ] : sampler.output
		...
		return Promise.all( [
			...
			for ( let i = 0, il = nodes.length; i < il; i ++ ) {
				...
				switch ( PATH_PROPERTIES[ target.path ] ) {
					...
					case PATH_PROPERTIES.rotation:
						TypedKeyframeTrack = M3DREP.quaternionKeyframeTrack
						break
					case PATH_PROPERTIES.position:
					case PATH_PROPERTIES.scale:
					default:
						TypedKeyframeTrack = M3DREP.vectorKeyframeTrack
						break
				}

				var targetName = node.name ? node.name : node.uuid,

				interpolation = sampler.interpolation !== undefined ? INTERPOLATION[ sampler.interpolation ] : M3DREP.interpolateLinear,
			...
			return new M3DREP.animation.clip( name, undefined, tracks )
		} )
	}
	 ...
	 * @return {Promise<M3DREP.object3D>}
	 */
	GLTFParser.prototype.loadNode = function ( nodeIndex ) {
		...
		return ( function () {
			...
			if ( nodeDef.isBone === true ) node = new M3DREP.bone()
			else if ( objects.length > 1 ) node = new M3DREP.group()
			else if ( objects.length === 1 ) node = objects[ 0 ]
			else node = new M3DREP.object3D()
			...
			return parser.getDependency( 'node', nodeId ).then( function ( node ) {
				...
				return parser.getDependency( 'skin', nodeDef.skin ).then( function ( skin ) {
					...
					node.traverse( function ( mesh ) {
						...
						mesh.bind( new M3DREP.skeleton( bones, boneInverses ), mesh.matrixWorld )
					} )
