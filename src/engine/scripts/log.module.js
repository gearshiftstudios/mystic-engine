export class Log {
    constructor ( category, isWorker, isEngine ) {
        ...
        this.isEngine = !isEngine ? false : ( isEngine != true ) ? 'Program' : false
    }
        ...
        return {
            reg: () => {
                verify().then( console.log( 
                    `%c ${ ( this.isEngine != true ) ? program.name : `${ 'Mystic Engine' } v${ 0.1 }` } %c ${ this.category }${ ( this.isWorker == true ) ? ' Worker' : '' } %c${ !subject ? '' : ( typeof subject != 'string' ) ? '' : ` ${ subject } ` }%c ${ content }`,
            ...
            error: line => {
                verify().then( console.error( 
                    `%c ${ ( this.isEngine != true ) ? program.name : `${ 'Mystic Engine' } v${ 0.1 }` } %c ${ this.category }${ ( this.isWorker == true ) ? ' Worker' : '' } %c${ !subject ? '' : ( typeof subject != 'string' ) ? '' : ` ${ subject } ` }%c${ !line ? '' : ( typeof line != 'number' ) ? '' : ` ${ line } ` }%c ${ content }`,
                    `background: ${ ( this.isEngine == true ) ? 'black' : 'blue' }; color: ${ ( this.isEngine == true ) ? 'turquoise' : 'white' }; font-weight: bold;`,
                    `background: ${ ( this.isWorker == true ) ? 'yellow' : 'white' }; color: black; font-weight: bold;`,
                    `background: magenta; color: white; font-weight: bold;`,
